	...
	while (!glfwWindowShouldClose(window)) {
		...
		*/
		...
		if (sbuffer != "")
		{
			...
			int clientID = std::stoi(tempBuffer);
			if (clientID == 1 && client1.clientID == 0)
			{
				client = &client1;
				client1.sockAddr = fromAddr;
				client1.clientID = clientID;
			}
			else if (clientID == 2 && client2.clientID == 0)
			{
				client = &client2;
				client2.sockAddr = fromAddr;
				client2.clientID = clientID;
			}
			if (clientID == 1)
			{
				client = &client1;
		...
		client2.Model = glm::translate(client2.Model, glm::vec3(client2.tx, client2.ty, -4.0f));
		//Model = glm::
		mvp = Projection * View * client2.Model;
		...
		ball.Model = glm::translate(ball.Model, glm::vec3(ball.tx, ball.ty, -4.0f));
